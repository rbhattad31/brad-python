...
def generate_domestic_and_import_wise(in_config):
    ...
    try:
        ...
        read_excel_pd_2 = pd.read_excel(in_config["ExcelPath"], sheet_name=in_config["Q3 Sheet_Dom&Imp"], skiprows=5)
        ...
        read_excel_pd_2 = read_excel_pd_2[["Currency Key", "GR Amt.in loc.cur."]]
        read_excel_pd_2 = read_excel_pd_2.dropna()

        # create a new column 'Purchase Type' with blank value
        read_excel_pd_2['Purchase Type'] = ''

        # Setting Type of purchase column values using currency key column on condition
        read_excel_pd_2.loc[read_excel_pd_2['Currency Key'] == "INR", 'Purchase Type'] = "Domestic"
        read_excel_pd_2.loc[read_excel_pd_2['Currency Key'] != "INR", 'Purchase Type'] = "Import"

        # read previous quarters final working file
        previous_quarter_final_file_pd = pd.pivot_table(read_excel_pd_2, index=["Purchase Type"],
                                                     values="GR Amt.in loc.cur.",
                                                     aggfunc=numpy.sum, margins=True, margins_name="Grand Total")
        ...
        domestic_and_import_wise_comparatives_pd.to_excel(in_config["Dom&Imp_Path"],
                                                          sheet_name=in_config["Dom&Imp Sheet"], index=False, startrow=16)
